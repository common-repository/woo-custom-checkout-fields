...
class wcfew_update_checkout_fields {
	 ...
	 public function validate_additional_required_fields() {
		 ...
		 $requiredtext      =  __('is a required field','wcfew');
       ...
       if (isset($additional_fields) && (sizeof($additional_fields) >= 1)) { 
        ...
        foreach ($additional_fields as $key=>$value) {
			 if (isset($value['visibility'])) {
				 ...
				 $is_field_hidden=$this->wcfew_check_if_field_is_hidden($visibilityarray,$allowedproducts,$allowedcats);
	...
	public function wcfew_check_if_field_is_hidden($hiddenvalue,$allowedproduts ,$allowedcats ) {
		global $woocommerce;
		$cart_items = $woocommerce->cart->get_cart();
		$extra_options = (array) get_option( 'wcfew_extra_settings' );
...
new wcfew_update_checkout_fields();
?>